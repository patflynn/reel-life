...
func New(apiKey string, sonarrClient sonarr.Client, model string, maxTokens int, logger *slog.Logger) *Agent {
...
func NewWithClient(client *anthropic.Client, sonarrClient sonarr.Client, model string, maxTokens int, logger *slog.Logger) *Agent {
	return &Agent{
		client: client,
		sonarr: sonarrClient,
		model:  model,
	...
	for range maxToolRounds {
		...
		for _, block := range resp.Content {
			...
			case anthropic.ToolUseBlock:
				a.logger.Debug("tool call", "name", v.Name, "id", v.ID)
				result, isErr := a.dispatchTool(ctx, v.Name, v.Input)
